#!/usr/bin/env node
/**
 * Patch to reduce system reminder token overhead (2.1.139)
 *
 * 2.1.139: task reminder copy lost the "NEVER mention this reminder to the user"
 * tail; new closing is "ignore if not applicable." Pattern updated accordingly.
 *
...
if (TASK_REMINDER !== 'keep') {
  ...
  if (TASK_REMINDER === 'remove') {
    // 2.1.209: the gate gained a second condition — if(!MR()||vJ())return[]
    // (was a single if(!X())return[]). Match both gate fns; rest of the body
    // is unchanged.

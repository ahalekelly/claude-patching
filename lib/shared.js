...
function detectBareInstall() {
  // pnpm ≥10 puts shims in bin/; older pnpm dropped them directly in the
  // share root. Try the new location first, fall back to the legacy one.
  const pnpmRoot = path.join(os.homedir(), '.local/share/pnpm');
  const candidates = [
    path.join(pnpmRoot, 'bin/claude'),
    path.join(pnpmRoot, 'claude'),
  ];
  const wrapperPath = candidates.find(p => fs.existsSync(p));
  ...
  try {
    ...
    // Resolve the exec line. pnpm shims use `$basedir/<relpath>` (legacy root
    // shim) or `$basedir/../<relpath>` (bin/ shim). Capture either.
    const execMatch = wrapperContent.match(
      /exec\s+"?\$basedir\/((?:\.\.\/)?[^"\s]+\/bin\/claude\.exe)"?/
    );
    if (execMatch) {
      const binaryPath = path.resolve(wrapperDir, execMatch[1]);
      if (fs.existsSync(binaryPath) && isElfFile(binaryPath)) {
        return {
          type: 'bare',
          path: binaryPath,
          version: readPackageVersion(binaryPath) ?? extractPnpmVersion(binaryPath) ?? 'unknown',
        };
      }
    }

    // Legacy layout (≤2.1.116): package shipped cli.js directly. The shim
    // exec'd it via node so the line shape differs (no `exec` prefix).
    const legacyMatch = wrapperContent.match(
      /\$basedir\/((?:\.\.\/)?[^"\s]+\/cli\.js)/
    );
    if (legacyMatch) {
      const cliPath = path.resolve(wrapperDir, legacyMatch[1]);
      if (fs.existsSync(cliPath)) {
        return {
          type: 'bare',
          path: cliPath,
          version: readPackageVersion(cliPath) ?? extractPnpmVersion(cliPath) ?? 'unknown',
        };
      }
    }
  } catch (err) {
...
/**
 * Read the version from the nearest package.json above a binary path.
 * Used for the post-pnpm-10 layout where the version is no longer in the path.
 */
function readPackageVersion(binaryPath) {
  try {
    // .../node_modules/@anthropic-ai/claude-code/bin/claude.exe
    //                                            ^^^ pkg dir is two up
    const pkgPath = path.join(path.dirname(binaryPath), '..', 'package.json');
...
/**
 * Legacy fallback: pre-pnpm-10 layout encoded the version in the .pnpm dir.
 */
function extractPnpmVersion(p) {
  const m = p.match(/@anthropic-ai\+claude-code@([^/]+)/);
  return m ? m[1] : null;
}
...
function listRecentBaks(installType, targetPath, limit = 3) {
  ...
  try {
    if (installType === 'native') {
      ...
      // .../cli.js (legacy). Two layouts to handle:
      //   pre-pnpm-10:  .../.pnpm/@anthropic-ai+claude-code@<ver>/node_modules/...
      //   pnpm ≥10:     .../global/<vN>/<hash>/node_modules/...
      // In both cases per-version package dirs are siblings; walk and probe.
      const legacyPnpm = targetPath.match(/^(.+\/\.pnpm)\/(@anthropic-ai\+claude-code@[^/]+)\/(.+)$/);
      const modernPnpm = targetPath.match(/^(.+\/global\/[^/]+)\/([^/]+)\/(node_modules\/@anthropic-ai\/claude-code\/.+)$/);
      const layout = legacyPnpm || modernPnpm;
      if (layout) {
        const parentDir = layout[1];
        const relPath = layout[3];
        ...
        const entries = fs.readdirSync(parentDir).filter(d => {
          if (legacyPnpm) return d.startsWith('@anthropic-ai+claude-code@');
          const full = path.join(parentDir, d);
          try { return fs.statSync(full).isDirectory(); } catch { return false; }
        });
        for (const entry of entries) {
          for (const rel of candidates) {
            const candidate = path.join(parentDir, entry, rel + '.bak');
      ...
      .map(full => {
        ...
        if (installType === 'bare') {
          ...
          if (verMatch) {
            ...
            // pnpm ≥10: no version in path, pull it from sibling package.json
            const ver = readPackageVersion(full.replace(/\.bak$/, ''));
            if (ver) name = `${ver}.bak`;
          }
        }
...
// what upstream's bundler emits, and it's the only form that survives CC's
// module loading. A raw UTF-8 char written into the JS decodes as latin1
// downstream and renders as mojibake, with no syntax error to catch it. So:
//
//   a patch must never RAISE the non-ASCII count of the bundle.
//
// Checked on the output rather than on patch sources, because nearly every
// patch file carries em-dashes in its comment header — those never reach the
// bundle, and linting them would be pure noise.
// ============================================================

/**
 * Tally non-ASCII code units by character.
 * @param {string} str
 * @returns {Map<string, number>}
 */
function nonAsciiCounts(str) {
...
/**
 * Find non-ASCII characters that `after` gained relative to `before`, with a
 * source snippet locating each one.
 *
 * A gained character is located by scanning `after` for a window around each of
 * its occurrences and keeping the first window absent from `before` — which is
 * the newly-injected site rather than a pre-existing one.
 *
 * @param {string} before - content prior to patching
 * @param {string} after - content after patching
 * @param {{ limit?: number, window?: number }} [opts]
 * @returns {Array<{ char: string, code: string, added: number, context: string }>}
 */
function findGainedNonAscii(before, after, opts = {}) {
  const { limit = 5, window = 40 } = opts;
  const beforeCounts = nonAsciiCounts(before);
  const afterCounts = nonAsciiCounts(after);

  const gained = [];
  for (const [ch, count] of afterCounts) {
    const added = count - (beforeCounts.get(ch) || 0);
    if (added <= 0) continue;

    let context = '';
    for (let idx = after.indexOf(ch); idx !== -1; idx = after.indexOf(ch, idx + 1)) {
      const snippet = after.slice(Math.max(0, idx - window), idx + window);
      if (!before.includes(snippet)) {
        context = snippet;
        break;
      }
    }
  ...
  return gained.sort((a, b) => b.added - a.added).slice(0, limit);
}

...
- **Agent**: headless Opus — `<versions-binary> -p --model opus`, invoked by absolute path per the recursion guard — cwd `~/.agents/claude-patching`, unattended so it needs `--dangerously-skip-permissions`. Its blast radius is bounded by the promotion gate below, not by permissions.

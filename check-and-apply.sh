...
if [[ "$SELECTED" != "$BIN" ]]; then
  ...
  message "claude-patching: $VER is not patched yet — launching patched $FALLBACK from the archive."
  ...
  if [[ "${VER%.*}" != "${FALLBACK%.*}" || $behind -gt 3 || $days -gt 7 ]]; then
    message "claude-patching: WARNING — that is $behind release(s) and $days day(s) behind $VER. Check $STATE/port-$VER.log."
  fi
else
  FALLBACK="stock $VER"
  message "claude-patching: no patched binary available — launching stock $VER."
fi
[[ -n "$brief" ]] && message "$brief"

# Report the markers before the port below rewrites them, then reconcile in
# the background. Never blocks the launch, and self-damps if the port of this
# version already failed recently.

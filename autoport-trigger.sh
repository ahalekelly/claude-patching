#!/usr/bin/env bash
# Fired by launchd on every change to Claude Code's versions directory, and once
# at load. Claude Code's auto-updater installs new versions on its own schedule
# and the daemon starts spawning sessions on them immediately, so reconciliation
# has to be driven by the install rather than by the next interactive launch.
set -uo pipefail
...
# check-and-apply.sh and background-port.sh.
...
# exec rather than spawn: launchd owns this pid, so the port it becomes is the
# job launchd tracks, and ThrottleInterval keeps a burst of watch events from
# stacking ports on top of each other. The port's own damper handles a version
# whose port already failed.
mkdir -p "$STATE"

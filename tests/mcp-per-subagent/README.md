...
- `run.py <binary>` — the gate test. Drives a headless session that launches `expa` twice concurrently and asserts on the log. It carries the `expa` definition, including its inline stdio `mcpServers` entry, and hands it to the session as `--agents` JSON: Claude Code drops inline `mcpServers` from an agent defined in a project directory unless that project is trusted, and agents defined on the command line are exempt, so the fixture needs no trust state in the config it shares with the real user. Two concurrent invocations compute byte-identical configs, which is the condition that triggers stock's connection dedup. The prompt each invocation receives names its role: `waiter` pings, waits for the quick probe's server to shut down, then pings again; `quick` waits for the waiter's first ping before doing anything.
